...
export default function MfaTotpSetup({
      ...
      <form onSubmit={onEnable} className="space-y-5 animate-in slide-in-from-right-4">
        ...
        {!isGoogleUser ? (
          ...
          <div className="bg-gray-50 dark:bg-slate-800/60 p-5 rounded-2xl border border-gray-100 dark:border-slate-700 text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed mb-1">
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4">
      <div className="p-5 bg-gray-50 dark:bg-slate-800/60 rounded-2xl border border-gray-100 dark:border-slate-700 flex flex-col items-center justify-center text-center">
        ...
        <div className="bg-gray-200/50 dark:bg-slate-800 px-4 py-2 rounded-lg mt-2 w-full max-w-full">
          <p className="text-[10px] text-gray-500 dark:text-slate-400 uppercase font-bold tracking-wider mb-1">Manual Entry Key</p>
          <p className="font-mono text-sm font-bold text-gray-800 dark:text-slate-200 tracking-widest break-all">{extractSecret(totpURI)}</p>
        </div>
      </div>

      {backupCodes && backupCodes.length > 0 && (
        <div className="p-5 bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/50 rounded-2xl">
          <p className="text-sm font-bold text-red-900 dark:text-red-200 mb-1">2. Save Backup Codes</p>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm text-red-900 dark:text-red-200 font-mono mb-4">
            {backupCodes.map((code) => (
              <div key={code} className="bg-white/80 dark:bg-slate-900 px-3 py-1.5 rounded text-center font-bold tracking-widest border border-red-100 dark:border-red-900/50 shadow-sm">{code}</div>
            ))}
          </div>
          ...
          <label className="flex items-start gap-3 mt-4 cursor-pointer group">
            <div className="relative flex items-center justify-center pt-0.5">
              ...
              />
              <div className="w-5 h-5 border-2 border-red-200 dark:border-red-800 rounded peer-checked:bg-red-600 peer-checked:border-red-600 transition-colors"></div>
            ...
            <span className="text-sm font-semibold text-red-900 dark:text-red-200 group-hover:text-red-700 dark:group-hover:text-red-100 transition-colors">
      ...
      <button
        ...
        className="w-full rounded-xl bg-gray-900 dark:bg-red-600 text-white px-6 py-3.5 text-sm font-bold hover:bg-black dark:hover:bg-red-700 transition-colors active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
      >
