...
export default function ModulePlayerPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex bg-gray-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="md:hidden absolute top-0 left-0 right-0 flex items-center justify-between bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 p-4 z-[120] shadow-sm">
        <button onClick={onClose} className="text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100 transition flex items-center gap-2 text-sm font-semibold">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
          Exit Preview
        </button>
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-gray-100 dark:bg-slate-800 rounded-lg text-gray-700 dark:text-slate-200 hover:bg-gray-200 dark:hover:bg-slate-700 transition">
          <MenuIcon />
        </button>
      </div>
      ...
      <div className="flex w-full h-full pt-[68px] md:pt-0 relative z-[115]">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-900 relative">
          ...
          {/* Admin Notice Banner */}
          <div className="bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border-b border-amber-200 dark:border-amber-900/50 px-4 py-2 text-center text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 shadow-sm z-[110]">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
            Live Player Preview Mode
            <button onClick={onClose} className="ml-2 bg-amber-900 dark:bg-amber-800 text-amber-50 px-2 py-0.5 rounded-full hover:bg-amber-950 dark:hover:bg-amber-700 transition">Close Preview</button>
          </div>
