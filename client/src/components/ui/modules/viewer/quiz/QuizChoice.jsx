export default function QuizChoice({ opt, isSelected, hasSubmitted, onChoiceClick, selectionOrder = 0 }) {
  let baseClasses = "w-full text-left p-4 rounded-xl border-2 transition-all duration-300 focus:outline-none flex items-center gap-4";
  
  if (!hasSubmitted) {
    if (isSelected) {
       baseClasses += " border-purple-600 bg-purple-50 text-gray-900 font-bold shadow-sm cursor-pointer";
    } else {
  ...
  return (
    ...
    >
      <div className="flex-1 text-[15px]">{opt.text}</div>
      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all duration-300 ${
        hasSubmitted 
          ...
          : (isSelected ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-300 bg-transparent')
      }`}>
