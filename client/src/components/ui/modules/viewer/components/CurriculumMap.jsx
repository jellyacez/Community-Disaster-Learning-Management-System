  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-4">
      ...
      <div className="text-center mb-14 md:mb-20">
        <h1 className="text-4xl md:text-6xl font-black text-gray-900 mb-3 tracking-tighter">
          Curriculum Map
        </h1>
      ...
      <div className="relative">
        ...
        <div className="absolute left-6 md:left-1/2 top-8 bottom-8 w-1 bg-gray-200 rounded-full -ml-0.5 md:ml-0 md:-translate-x-1/2 z-0 opacity-60" />
          ...
          {levels.map((lvl, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute left-6 md:left-1/2 -ml-6 md:ml-0 md:-translate-x-1/2 w-12 h-12 rounded-full border-[3px] flex items-center justify-center bg-white shadow-md z-20 transition-all ${
                    isLevelUnlocked
                      ? "border-red-600 text-red-600 ring-4 ring-red-50"
  ...
  return (
    <div
      className={`w-full max-w-sm sm:max-w-md h-56 sm:h-72 rounded-[2rem] p-3 transition-all duration-300 ${
        isLevelUnlocked
          ? "bg-white/80 border border-gray-200/80 shadow-md hover:shadow-xl hover:border-red-200 group"
          : "bg-gray-50/50 border border-gray-200 opacity-60 grayscale"
      }`}
    >
      <div className="w-full h-full rounded-[1.5rem] overflow-hidden relative flex flex-col items-center justify-center bg-gradient-to-br from-red-500/10 via-rose-500/5 to-amber-500/10 border border-black/5">
        {coverImg ? (
          <>
            <img
              src={coverImg}
          ...
          <div className="flex flex-col items-center justify-center p-6 text-center space-y-3">
            ...
            <div>
              <span className="text-[11px] font-black uppercase tracking-widest text-red-600/80">
  ...
  return (
    <div
      className={`w-full max-w-sm sm:max-w-md p-6 sm:p-7 rounded-[2rem] border transition-all duration-300 ${
        isLevelUnlocked
          ? "bg-white border-gray-200 hover:shadow-xl hover:border-red-200 shadow-sm"
          : "bg-gray-50/50 border-gray-200 opacity-70"
      }`}
    >
      {/* Title & Phase Badge Header */}
      <div className="flex items-center gap-3 mb-3">
        <div className="px-3 py-1 rounded-xl bg-red-50 text-red-700 border border-red-100 font-black text-xs uppercase tracking-wider">
          Phase {lvl.level_order || idx + 1}
        </div>
        <h3
          className={`text-xl sm:text-2xl font-black tracking-tight truncate ${
            isLevelUnlocked ? "text-gray-900" : "text-gray-500"
          }`}
        >
          {decodeHtml(lvl.title)}
        </h3>
      ...
      {lvl.description && (
        <div
          className="text-xs sm:text-sm font-medium text-gray-500 mb-5 leading-relaxed prose prose-sm max-w-none [&>p]:m-0"
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3.5 rounded-2xl flex items-center gap-3.5 text-sm transition-all duration-200 ${
                isStepLocked
                  ...
                  : isCompleted
                  ? "bg-emerald-50 text-emerald-900 hover:bg-emerald-100 border border-emerald-100 shadow-xs cursor-pointer"
                  : "bg-red-50/60 text-red-950 hover:bg-red-100 font-bold border border-red-100 shadow-xs hover:shadow-md cursor-pointer"
              }`}
            >
              <div
