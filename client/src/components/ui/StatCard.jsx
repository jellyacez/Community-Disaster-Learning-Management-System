  ...
  const CardContent = (
    <div
      ...
      className={`${bgTintMap[color] || "bg-white"} rounded-2xl shadow-[0_2px_12px_-3px_rgba(0,0,0,0.06)] p-4 flex flex-col gap-3 h-full border ${
        isClickable
          ? "hover:-translate-y-1 hover:shadow-[0_8px_20px_-4px_rgba(0,0,0,0.1)] hover:border-red-200 cursor-pointer transition-all duration-300"
          : ""
      } ${
        isActive
          ? "ring-2 ring-red-500 border-red-500 bg-red-50/10"
          : "border-gray-100/80"
      }`}
    >
      <div className="flex items-start justify-between">
        <div
          className={`w-14 h-14 rounded-full flex items-center justify-center ${iconColorMap[color]} bg-opacity-60 transition-opacity ${
            ...
            {suffix && (
              <span
                className={`text-xl lg:text-2xl font-bold ml-1 ${
                  isZero ? "text-gray-300" : "text-gray-500"
                }`}
              >
                {suffix}
              </span>
