...
function ToggleSwitch({ enabled, onChange, disabled = false }) {
  return (
    <button
      ...
      disabled={disabled}
      className={`${enabled ? 'bg-red-600 dark:bg-red-600' : 'bg-gray-300 dark:bg-slate-600'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2`}
    >
      <span className={`${enabled ? 'translate-x-5' : 'translate-x-0'} pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white dark:bg-white shadow ring-0 transition duration-200 ease-in-out`} />
    </button>
  );
}
    ...
    <div className="p-6 md:p-8 w-full flex flex-col space-y-2">
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 p-4 -mx-4 rounded-2xl hover:bg-gray-50/80 dark:hover:bg-slate-800/40 transition-colors group">
        <div className="md:w-1/3 shrink-0">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Notify me about new Bacolor municipality announcements.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 p-4 -mx-4 rounded-2xl hover:bg-gray-50/80 dark:hover:bg-slate-800/40 transition-colors group">
        <div className="md:w-1/3 shrink-0">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Receive active disaster warnings <span className="text-red-500 dark:text-red-400 font-medium">(Locked for safety)</span>.</p>
        </div>
        <div className="md:w-2/3 max-w-md flex items-center justify-end">
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 p-4 -mx-4 rounded-2xl hover:bg-gray-50/80 dark:hover:bg-slate-800/40 transition-colors group">
        <div className="md:w-1/3 shrink-0">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Get reminders for incomplete learning modules.</p>
        </div>
        <div className="md:w-2/3 max-w-md flex items-center justify-end">
          <ToggleSwitch 
