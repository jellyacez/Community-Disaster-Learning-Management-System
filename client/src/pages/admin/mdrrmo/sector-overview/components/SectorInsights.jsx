  ...
  if (isLoading) {
    return (
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-28 rounded-2xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-5 animate-pulse shadow-sm flex flex-col justify-between">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-gray-200 dark:bg-slate-800" />
              <div className="h-3.5 w-24 bg-gray-200 dark:bg-slate-800 rounded-full" />
            </div>
            <div className="space-y-1.5 mt-2">
              <div className="h-4 w-3/4 bg-gray-200 dark:bg-slate-800 rounded-full" />
              <div className="h-3 w-1/2 bg-gray-100 dark:bg-slate-800/60 rounded-full" />
            </div>
          </div>
        ))}
      </div>
