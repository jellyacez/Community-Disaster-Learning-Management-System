...
export default function LevelSelector({ 
  ...
  const handleAddLevel = () => {
    const nextOrder = stagedLevels.reduce((max, lvl) => Math.max(max, lvl.levelOrder), 0) + 1;
    setStagedLevels([
      ...stagedLevels,
      { 
  ...
  const handleUpdateLevel = (levelOrder, field, value) => {
    ...
    setStagedLevels(prev => 
      prev.map(lvl => lvl.levelOrder === levelOrder ? { ...lvl, [field]: finalValue } : lvl)
    );
  };

  const activeLevelData = stagedLevels.find(l => l.levelOrder === activeLevelOrder);
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4 animate-in fade-in duration-150">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Curriculum Phases & Levels</h3>
          <p className="text-[11px] text-slate-500 font-medium mt-0.5">Organize items into separate level blocks linked to this module.</p>
        </div>
        <div className="flex items-center gap-2">
          {activeLevelOrder === 1 ? (
            ...
            <button
              type="button"
              onClick={() => {
                const lvl = stagedLevels.find(l => l.levelOrder === activeLevelOrder);
                if (lvl) setLevelToDelete(lvl);
              }}
      ...
      {activeLevelData && (
        <div key={activeLevelData.levelOrder} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-slate-50 border border-slate-200 rounded-xl animate-in fade-in duration-200">
          <div className="space-y-4 md:col-span-2">
            <div>
              ...
              <input
                ...
                onChange={(e) => handleUpdateLevel(activeLevelData.levelOrder, 'levelTitle', e.target.value)}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleUpdateLevel(activeLevelData.levelOrder, 'levelDescription', e.target.value)}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Passing Threshold (%)</label>
            <input
              type="number"
              min="0"
              max="100"
              value={activeLevelData.passing_threshold || 0}
              onChange={(e) => handleUpdateLevel(activeLevelData.levelOrder, "passing_threshold", e.target.value)}
          ...
          <div className="flex items-center space-x-3 mt-4">
            <input
              ...
              onChange={(e) => handleUpdateLevel(activeLevelData.levelOrder, "is_locked_by_default", e.target.checked)}
              className="w-5 h-5 text-red-600 border-slate-300 rounded focus:ring-red-500 cursor-pointer"
            />
            <label htmlFor={`lock-default-${activeLevelData.levelOrder}`} className="text-sm font-bold text-slate-700 cursor-pointer select-none">
      ...
      <ConfirmationModal
          ...
          if (levelToDelete) {
            if (levelToDelete.levelOrder === 1) {
              ...
              const updatedLevels = stagedLevels.map(l => 
                l.levelOrder === 1 
                  ? { ...l, levelTitle: "", levelDescription: "", passing_threshold: 80, is_locked_by_default: false } 
                  : l
              );
              setStagedLevels(updatedLevels);
              
              if (setStagedFlows) {
                const updatedFlows = stagedFlows.filter(f => f.levelOrder !== 1);
                setStagedFlows(updatedFlows);
              }
              setLevelToDelete(null);
            } else {
              // Delete the level
              const updatedLevels = stagedLevels.filter(l => l.levelOrder !== levelToDelete.levelOrder);
              setStagedLevels(updatedLevels);
              
              // Delete all steps associated with this level
              if (setStagedFlows) {
                const updatedFlows = stagedFlows.filter(f => f.levelOrder !== levelToDelete.levelOrder);
                setStagedFlows(updatedFlows);
              }
        ...
        title={levelToDelete?.levelOrder === 1 ? "Clear Level 1" : "Delete Level"}
        description={
        ...
        confirmText={levelToDelete?.levelOrder === 1 ? "Clear Level 1 Data" : "Delete Level & Steps"}
        type="danger"
      />
    </div>
  );
