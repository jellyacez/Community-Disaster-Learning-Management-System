...
export default function SequenceCard({
  ...
  return (
    <div className="w-full flex flex-col items-center relative group">
      ...
      <div className="h-10 border-l-2 border-dashed border-gray-300 group-hover:border-gray-400 transition-colors"></div>
      ...
      <div 
        ...
        className="w-full flex items-start gap-4 p-5 bg-white border border-gray-200 rounded-[1.5rem] shadow-sm hover:shadow-md hover:border-gray-300 transition-all cursor-grab active:cursor-grabbing relative"
      >
        <div className="absolute left-[-4rem] top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5">
          <div className="flex flex-col gap-0.5">
            <button onClick={(e) => { e.stopPropagation(); moveFlowStep(index, "up"); }} disabled={index === 0} className="p-1 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded disabled:opacity-30 transition-colors">
                <HugeiconsIcon icon={ArrowUp01Icon} className="w-3.5 h-3.5" />
            </button>
            <button onClick={(e) => { e.stopPropagation(); moveFlowStep(index, "down"); }} disabled={index === localizedFlowsLength - 1} className="p-1 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded disabled:opacity-30 transition-colors">
        ...
        <div className="flex-1 min-w-0">
            ...
            {flow.textContent && (
              <p className="text-sm text-gray-600 mt-3 line-clamp-1">{extractTextFromHTML(flow.textContent)}</p>
            )}
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0 ml-4">
            ...
            <div className="relative flex items-center gap-1 border-l border-gray-200 pl-3 ml-1">
