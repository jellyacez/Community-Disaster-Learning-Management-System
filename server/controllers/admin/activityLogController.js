...
exports.getMdrrmoActivityLog = async (req, res) => {
  try {
    ...
    // Base condition: Exclude system_admin events
    ...
    if (action) {
      if (action === 'auth') {
        ...
        conditions.push(`(al.act_log ILIKE $${paramIndex} OR al.act_log ILIKE $${paramIndex+1})`);
        params.push('%module%', '%certificate%');
        paramIndex += 2;
      } else {
        conditions.push(`al.act_log ILIKE $${paramIndex}`);
        params.push(`%${action}%`);
...
exports.exportMdrrmoActivityLog = async (req, res) => {
  try {
    ...
    require('../../utils/logger').logActivity(req.user.id, 'Exported MDRRMO sector activity logs');
